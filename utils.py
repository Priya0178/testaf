...
async def is_subscribed(bot, query):
    ...
    if db2().isActive() and REQ_CHANNEL:
...
async def get_poster(query: str, bulk=True, file=None):
...
async def google_search(text):
    ...
    rs = [title.get_text(strip=True) for title in titles][:5]
    logging.info(rs)
    return rs
...
def get_file_id(msg: Message):
    if msg.media:
        for message_type in (
            ...
            if obj:
                setattr(obj, "message_type", message_type.value)
                return obj
