...
from database.ia_filterdb import total_filter_Counts
...
async def save_group(bot, message):
    new_memebers = [u.id for u in message.new_chat_members]
    if temp.ME in new_memebers:
        if not await db.get_chat(message.chat.id):
            total = await bot.get_chat_members_count(message.chat.id)
            invitee = message.from_user.mention if message.from_user else "Anonymous" 
        ...
        if message.chat.id in temp.BANNED_CHATS:
            ...
            return
        buttons = [[
            InlineKeyboardButton('ℹ️ Help', url=f"https://t.me/Ct_Promo_bot"),
            InlineKeyboardButton('🧑‍🏫Tutorial', url='https://t.me/cinema_tharavadu')
        ]]
        reply_markup=InlineKeyboardMarkup(buttons)
...
async def get_ststs(bot, message):
    ...
    files = await total_filter_Counts()
